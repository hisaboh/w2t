...
class Zun{
	 ...
	 */
	static public function f($src,$html=true){
		/***
			$text = text('
						[[[
	...
	static private function isExclusion($value,$exclusion){
		if($exclusion === 0){
	...
	static private function toBr($src){
		$result = "";
		$exclusion = 0;
		$en = false;
		...
		foreach(explode("\n",$src) as $value){
			$en = true;
			$exclusion = self::isExclusion($value,$exclusion);
			$result .= (($exclusion > 0) ? $value : $value."<br />")."\n";
		}
		return ($en) ? substr($result,0,-7) : $result;
	}
	static private function toTable($src){
		$result = "";
		$exclusion = 0;
		$isTable = false;
		$en = false;
	...
	static private function toList($src){
		$result = "";
		$exclusion = 0;
		$indent = 0;
		$pre_indent = 0;
